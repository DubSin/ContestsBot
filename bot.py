            ...
            if 0 <= delta.total_seconds() <= time:
                if event[4] != '-' and event[2] != 'n':
                    ...
                    await bot.send_photo(HOOPS_ID, photo=image,
                                         caption='@all \n' + event[3] + f'\n 🎉🎉🎉И победитель нашего розыгрыша футболки YEEZY x Gap x Balenciaga - @{event[2]} 🎉🎉🎉

Поздравляем! 🥳

Скоро мы свяжемся с тобой, чтобы уточнить детали получения приза. 

Следи за нашими обновлениями, у нас еще много интересного! 😉')
...
Следи за нашими обновлениями, у нас еще много интересного! 😉')
                    ...
                    image = FSInputFile(f'photos/{event[4]}')
                    await bot.send_photo(HOOPS_ID, photo=image,
                                         caption='@all \n' + event[3] + f'\n 🎉🎉🎉И победитель нашего розыгрыша футболки YEEZY x Gap x Balenciaga - @{winner[1]} 🎉🎉🎉
