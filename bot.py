            ...
            if 0 <= delta.total_seconds() <= time:
                if event[4] != '-' and event[2] != 'n':
                    ...
                    await bot.send_photo(keys.HOOPS_ID, photo=image,
                                         caption=f'Победил: @{event[2]}' + event[-1])
                    ...
                    image = FSInputFile(f'photos/{event[4]}')
                    await bot.send_photo(keys.HOOPS_ID, photo=image,
                                         caption=f'Победил: @{winner[1]}' + event[-1])
                    await bot.send_photo(keys.ADMIN_ID, photo=image,
                                         caption=f'Победил: @{winner[1]}' + event[-1])
                    await del_photos(f'photos/{event[4]}')
                elif event[2] != 'n' and event[4] == '-':
                    await bot.send_message(keys.HOOPS_ID, f'Победил: @{event[2]}' + event[-1])
                    await bot.send_message(keys.ADMIN_ID, f'Победил: @{event[2]}' + event[-1])
                elif event[2] == 'n' and event[4] == '-':
                    winner = random.choice(members)
                    await bot.send_message(keys.HOOPS_ID, f'Победил: @{winner[1]}' + event[-1])
                    await bot.send_message(keys.ADMIN_ID, f'Победил: @{winner[1]}' + event[-1])
                bot_db.del_event()
